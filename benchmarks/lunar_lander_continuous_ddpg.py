...
def _define_ddpg_model(actor_config: Config, critic_config: Config) -> DeepDeterministicPolicyGradient:
    ...
    learning_rate_q_values: float = 1e-4
...
def _define_agent(model: DeepDeterministicPolicyGradient) -> DDPGAgent:
    ...
    batch_size: int = 128
    noise_scale_max: float = 0.35
    noise_scale_min: float = 0.0
    noise_decay: float = 1e-6
    # Return the agent
...
def run(workspace: str,
    ...
    # nn_config.add_hidden_layer(LayerType.batch_normalization, [], layer_name="batch_norm_inputs")
    nn_config.add_hidden_layer(LayerType.dense, [400, tensorflow.nn.relu, True, tensorflow.contrib.layers.xavier_initializer()], layer_name="dense_1")
    nn_config.add_hidden_layer(LayerType.dense, [300, tensorflow.nn.relu, True, tensorflow.contrib.layers.xavier_initializer()], layer_name="dense_2")
    # Define model
