...
from usienarl.agents import TD3Agent
...
def _define_td3_model(actor_config: Config, critic_config: Config) -> TwinDelayedDDPG:
    # Define attributes
    learning_rate_policy: float = 1e-6
    learning_rate_q_values: float = 1e-6
    discount_factor: float = 0.9999
    polyak_value: float = 0.995
    buffer_capacity: int = 10000
    # Return the model
    return TwinDelayedDDPG("model", actor_config, critic_config,
                           learning_rate_policy, learning_rate_q_values,
                           discount_factor, polyak_value, buffer_capacity)


def _define_agent(model: TwinDelayedDDPG) -> TD3Agent:
    # Define attributes
    ...
    # Return the agent
    return TD3Agent("td3_agent", model, update_every_steps, policy_delay, summary_save_every_steps, batch_size,
...
def run(workspace: str,
    ...
    environment_name: str = 'LunarLanderContinuous-v2'
    ...
    nn_config.add_hidden_layer(LayerType.batch_normalization, [], layer_name="batch_normalization_1")
    ...
    nn_config.add_hidden_layer(LayerType.batch_normalization, [], layer_name="batch_normalization_2")
    ...
    nn_config.add_hidden_layer(LayerType.batch_normalization, [], layer_name="batch_normalization_3")
    # Define model
    inner_model: TwinDelayedDDPG = _define_td3_model(actor_config=nn_config, critic_config=nn_config)
    # Define agent
    td3_agent: TD3Agent = _define_agent(inner_model)
    # Define experiment
    ...
    run_experiment(logger=logger, experiment=experiment_default,
                   ...
                   render_during_training=True, render_during_validation=True,
