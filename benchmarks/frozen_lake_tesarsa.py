...
try:
    from src.tabular_expected_sarsa_agent import TabularExpectedSARSAAgent
...
def _define_epsilon_greedy_agent(model: TabularExpectedSARSA, exploration_policy: EpsilonGreedyExplorationPolicy) -> TabularExpectedSARSAAgent:
    # Define attributes
    batch_size: int = 100
    # Return the agent
    return TabularExpectedSARSAAgent("tesarsa_egreedy_agent", model, exploration_policy, batch_size)


def _define_boltzmann_agent(model: TabularExpectedSARSA, exploration_policy: BoltzmannExplorationPolicy) -> TabularExpectedSARSAAgent:
    # Define attributes
    batch_size: int = 100
    # Return the agent
    return TabularExpectedSARSAAgent("tesarsa_boltzmann_agent", model, exploration_policy, batch_size)
...
if __name__ == "__main__":
    ...
    # Define agents
    tesarsa_epsilon_greedy_agent: TabularExpectedSARSAAgent = _define_epsilon_greedy_agent(inner_model, epsilon_greedy_exploration_policy)
    tesarsa_boltzmann_agent: TabularExpectedSARSAAgent = _define_boltzmann_agent(inner_model, boltzmann_exploration_policy)
    # Define experiments
    experiment_egreedy: BenchmarkExperiment = BenchmarkExperiment("eg_experiment", success_threshold, environment,
                                                                  tesarsa_epsilon_greedy_agent)
    experiment_boltzmann: BenchmarkExperiment = BenchmarkExperiment("b_experiment", success_threshold, environment,
                                                                    tesarsa_boltzmann_agent)
