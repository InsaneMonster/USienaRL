...
try:
    ...
    from usienarl.agents.deep_sarsa_agent_epsilon_greedy import DeepSARSAAgent
...
def _define_dsarsa_model(config: Config, error_clip: bool) -> DeepSARSA:
    # Define attributes
...
if __name__ == "__main__":
    ...
    nn_config.add_hidden_layer(LayerType.dense, [32, tensorflow.nn.relu])
    # Define models
    inner_model_huber: DeepSARSA = _define_dsarsa_model(nn_config, True)
    inner_model_mse: DeepSARSA = _define_dsarsa_model(nn_config, False)
    ...
    # Define agents
    dsarsa_epsilon_greedy_agent_huber: DeepSARSAAgent = _define_epsilon_greedy_agent(inner_model_huber, epsilon_greedy_exploration_policy)
    dsarsa_boltzmann_agent_huber: DeepSARSAAgent = _define_boltzmann_agent(inner_model_huber, boltzmann_exploration_policy)
    dsarsa_epsilon_greedy_agent_mse: DeepSARSAAgent = _define_epsilon_greedy_agent(inner_model_mse, epsilon_greedy_exploration_policy)
    dsarsa_boltzmann_agent_mse: DeepSARSAAgent = _define_boltzmann_agent(inner_model_mse, boltzmann_exploration_policy)
    # Define experiments
    experiment_egreedy_huber: BenchmarkExperiment = BenchmarkExperiment("eg_experiment_huber", success_threshold, environment,
                                                                        dsarsa_epsilon_greedy_agent_huber)
    experiment_boltzmann_huber: BenchmarkExperiment = BenchmarkExperiment("b_experiment_huber", success_threshold, environment,
                                                                          dsarsa_boltzmann_agent_huber)
    experiment_egreedy_mse: BenchmarkExperiment = BenchmarkExperiment("eg_experiment_mse", success_threshold, environment,
                                                                      dsarsa_epsilon_greedy_agent_mse)
    experiment_boltzmann_mse: BenchmarkExperiment = BenchmarkExperiment("b_experiment_mse", success_threshold, environment,
                                                                        dsarsa_boltzmann_agent_mse)
