...
class TemporalDifferenceModel(Model):
    """
    Base class for each Temporal Difference (TD) _model. It should not be used by itself, and should be extended instead.

    A TD _model is a _model which uses the Q-value to find the optimal policy at each step by temporal difference.
    It usually supports only discrete action spaces, but it depends on the _model.

    Attributes:
        - learning_rate: the lambda of the _model in training phase
        - discount_factor: the discount factor of the discounted future expected reward
        - outputs: the outputs of the _model (usually one-hot encoded in the size of the action space of the environment)
    """
    ...
    def train_initialize(self,
                         session: object):
        """
        Initialize the training (e.g. to setup the weights of the _model).
    ...
    def update_single(self,
        ...
        """
        Update the _model weights (thus training the _model) of only one step experience in the environment.
        ...
        :return: the loss of the _model after training, the absolute error after training and the updated _summary
        """
        # Empty method, definition should be implemented on a child class basis
        return None
    ...
    def update_batch(self,
        ...
        """
        Update the _model weights (thus training the _model) of a batch of step experiences in the environment.
    ...
    def get_output(self,
                   ...
                   state_current):
        """
        Get the outputs of the _model at the given current state.

        :param session: the session of tensorflow currently running
        :param state_current: the current state in the environment to get the output for
        :return: the outputs of the _model given the state_current
        """
    ...
    def get_inputs_name() -> str:
        """
        Get the inputs _name in the _model. It is the same for each _model of the same _model type.

        :return: the str defining the inputs _name for the _model
        """
        # Empty method, definition should be implemented on a child class basis
        return ''
    ...
    def get_outputs_name() -> str:
        """
        Get the outputs _name in the _model. It is the same for each _model of the same _model type.

        :return: the str defining the outputs _name for the _model
        """
        # Empty method, definition should be implemented on a child class basis
        return ''
