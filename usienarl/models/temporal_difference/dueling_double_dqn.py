...
class Estimator:
    """
    Estimator defining the real DDDQN model. It is used to define two identical models: target network and q-network.

    It is generated given the size of the observation and action spaces and the hidden layer config defining the
    hidden layers of the DDDQN.
    """
    ...
    def __init__(self,
        ...
        with tensorflow.variable_scope(self.scope):
            ...
            self._value = tensorflow.layers.dense(hidden_value_stream_output, 1, None,
                                                  kernel_initializer=tensorflow.contrib.layers.xavier_initializer(), name="value")
            self._advantage = tensorflow.layers.dense(hidden_advantage_stream_output, *action_space_shape, None,
                                                      kernel_initializer=tensorflow.contrib.layers.xavier_initializer(), name="advantage")
            ...
            self.outputs = self._value + tensorflow.subtract(self._advantage,
                                                             tensorflow.reduce_mean(self._advantage, 1, True), name="outputs")
...
class DuelingDoubleDQN(TemporalDifferenceModel):
    ...
    It is further enhanced by the update process of the q-value. In the DDDQN the q-network estimates the outputs given
    the current state, but the best predicted action (the index of the best predicted output) is chosen by the target
    network.

    Also, the DDDQN divide the output stream in advantage function and value function, making sure they are also identifiable
    ...
    def __init__(self,
        ...
        # Define Dueling Double DQN attributes
    ...
    def _define(self):
        ...
        self._q_network = Estimator(self._experiment_name + "/" + self.name + "/QNetwork",
                                    self.observation_space_shape, self.action_space_shape,
                                    self._hidden_layers_config)
        self._target_network = Estimator(self._experiment_name + "/" + self.name + "/TargetNetwork",
                                         self.observation_space_shape, self.action_space_shape,
                                         self._hidden_layers_config)
    ...
    def update_single(self,
                      ...
                      state_current: int, state_next: int, action: int, reward: float, sample_weight: float = 1.0):
        """
        Overridden method of QLearningModel class: check its docstring for further information.
        """
        ...
        if self._observation_space_type == SpaceType.discrete:
            ...
            outputs_next_q_network = session.run(self.outputs,
                                                 feed_dict={self._inputs: [state_next if state_next is not None
                                                                           else numpy.zeros(self.observation_space_shape, dtype=float)]})
            outputs_next_target_network = session.run(self._outputs_target,
