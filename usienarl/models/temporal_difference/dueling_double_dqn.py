...
class Estimator:
    """
    Estimator defining the real DDDQN _model. It is used to define two identical models: target network and q-network.
...
class DuelingDoubleDQN(TemporalDifferenceModel):
    """
    DDDQN (Dueling Double Deep Q-Network) _model. The _model is a deep neural network which hidden layers can be defined by a config
        ...
        if self._observation_space_type == SpaceType.discrete:
            ...
            outputs_next_q_network = session.run(self.outputs,
                                                 feed_dict={self._inputs: [state_next if state_next is not None
                                                                           else numpy.zeros(self._observation_space_shape, dtype=float)]})
            outputs_next_target_network = session.run(self._outputs_target,
