...
class Agent:
    ...
    Usually, an agent is built for at least a model class, often it also employs one or more classes of
    ...
    Within this method the agents generates the model and connects to the environment by the interface.
    ...
    Each agent consist mainly of three different phases:
        ...
        - training, in which the inner model of the agent is updated and usually an exploration policy is employed
        - inference, where the model runs in full exploitation, usually used for validating and testing an agent
    ...
    The train phase comprehends:
        ...
        at the end of the episode by checking the relative flag.
        - update: the agent is invoked to update its inner model. When this is called depends on the experiment
    ...
    Attributes:
        - name: name of the agent, used as further scope with respect to the experiment
        - environment: the environment in which the agent is called to act, set by the experiment during setup
        - summary_writer: the tensorboard summary writer for the inner model of the agent
    ...
    def setup(self,
        ...
        :param scope: the experiment scope encompassing the agent scope, if any
    ...
    def act_train(self,
                  ...
                  agent_observation_current: numpy.ndarray):
        """
        Take an action given the current agent observation in warmup mode. Usually it uses an exploring policy.
    ...
    def act_inference(self,
                      ...
                      agent_observation_current: numpy.ndarray):
        """
        Take an action given the current agent observation in warmup mode. Usually it uses the best possible policy.
    ...
    def finish_step_warmup(self,
        ...
        """
        Finish a warmup step with the given values.
    ...
    def finish_episode_warmup(self,
                              ...
                              warmup_episode_volley: int):
        """
        Finish a warmup episode with the given values.
